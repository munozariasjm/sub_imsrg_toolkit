...
output_file = f"/work/submit/abelley/results/{Nucl}_c2_radii.csv"
...
class IMSRGResultsDF():
  ...
  def intialize_df(self, file = None):
    if file == None:
      ...
      try:
        self.df = pd.read_csv(file, dtype={'emax':int})
        names = ['Sample', 'emax',] 
...
def Rp2_to_Rch2( Rp2, Z, N, CODATA=True ):
    ...
    """
    if(CODATA):
        rcp2 = 0.707 # Updated CODATA
        rcn2 = -0.116    # Updated CODATA
    else:
...
for name in glob.glob(f"{directory}*.csv"):
  ...
  try:
    ...
    SmapleID = 0
    for param in params:
      if param[:9]=="100000000":
        SampleId = float(param)
      elif param.startswith('e'):
        emax = param.replace('e','')
        emax = int(emax)
    SampleId = int(float(SampleId)-1e10)
    LEC_cols = LECs.loc[176801].to_dict()
    LEC_cols['c2'] = c2s[SampleId]
